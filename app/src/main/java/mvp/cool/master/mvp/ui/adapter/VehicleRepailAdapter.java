...
public class VehicleRepailAdapter extends BaseQuickAdapter<PoiItem , BaseViewHolder>{

    public VehicleRepailAdapter(List<PoiItem> list){
        super(R.layout.adpater_repailvehicle , list);
    }
    ...
    protected void convert(BaseViewHolder holder, PoiItem item) {
        if(item != null){
            ...
            if(item.getPhotos().size() > 0){
                ...
                Glide.with(App.getInstance()).load(R.drawable.vehiclerepail)
                        .crossFade()
                        .into((ImageView)holder.getView(R.id.poiImage));
            }
        }
